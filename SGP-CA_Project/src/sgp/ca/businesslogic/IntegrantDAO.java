 ...
 public class IntegrantDAO implements IIntegrantDAO{
    ...
    public Integrant getIntegrantByUVmail(String emailUV){
        ...
        try{
            PreparedStatement sentenceQuery = CONNECTION.getConnectionDatabase().prepareStatement(
                "SELECT * FROM Integrant INNER JOIN Schooling WHERE emailUV = ?;"
            );
            ...
            integrant.setSchooling(this.getIntegrantStudies(queryResult));
    ...
    public void updateIntegrant(Integrant integrant, String oldRFC){
        ...
        try{
            ...
            PreparedStatement sentenceQuery = connection.prepareStatement(
                "UPDATE Integrant SET rfc = ?, bodyAcademyKey = ?, fullName = ?, dateOfAdmission = ?, emailUV = ?, "
            ...
            sentenceQuery.setString(6, integrant.getAditionalMail());
            sentenceQuery.setString(7, integrant.getCurp());
            sentenceQuery.setString(8, integrant.getNationality());
            sentenceQuery.setString(9, integrant.getEducationalProgram());
            sentenceQuery.setString(10, integrant.getCellphone());
            sentenceQuery.setInt(11, integrant.getStaffNumber());
            sentenceQuery.setString(12, integrant.getHomePhone());
            sentenceQuery.setString(13, integrant.getWorkPhone());
            sentenceQuery.setString(14, integrant.getAppointmentMember());
            sentenceQuery.setString(15, integrant.getParticipationType());
    ...
    public List<Schooling> getIntegrantStudies(ResultSet integrantResult) {
        List<Schooling> schooling = new ArrayList<>();
        try{
            while(integrantResult.next()){
                schooling.add(new Schooling(
                    integrantResult.getString("studyName"), 
                    integrantResult.getString("studiesInsitution"),
                    integrantResult.getString("levelOfStudy"),
                    integrantResult.getString("studiesState"),
                    integrantResult.getString("studyDiscipline"),
                    integrantResult.getString("studyArea"),
                    integrantResult.getDate("dateOfObtainingStudies").toString(),
                    integrantResult.getString("professionalID")
                ));
            }
        }catch(SQLException sqlException){
            Logger.getLogger(Integrant.class.getName()).log(Level.SEVERE, null, sqlException);
